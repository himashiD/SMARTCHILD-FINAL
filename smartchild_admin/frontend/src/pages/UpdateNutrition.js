// src/pages/UpdateNutrition.js
import React, { useEffect, useMemo, useRef, useState } from "react";
import axios from "axios";
import { useNavigate, useParams } from "react-router-dom";
import "../styles/addnutrition.css"; // ✅ reuse the earlier design

const API = process.env.REACT_APP_API || "http://localhost:5000";

export default function UpdateNutrition() {
  const { id } = useParams();
  ...
  const validate = () => {
    const ne = {};
    if (!form.title?.trim()) ne.title = "Title is required";
    if (!form.summary?.trim()) ne.summary = "Summary is required";
    if (!form.content?.trim()) ne.content = "Content is required";
    if (!form.published_date) ne.published_date = "Published date is required";
    setErrors(ne);
    ...
    (async () => {
      try {
        ...
        setForm({
          ...
          published_date: (data.published_date || "").slice(0, 10),
          status: data.status || "Published",
        });

        // show current media if backend returns filenames/paths
        if (data.image) setCurrentImage(`${API}/uploads/${data.image}`);
        if (data.document) {
          const fname = typeof data.document === "string" ? data.document : "";
          setCurrentDocName(fname.split("/").pop());
        }
      } catch (e) {
        console.error(e);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API}/nutrition/${id}`, fd, {
      ...
      console.error(err);
      alert("Failed to update nutrition guide.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="page-container">
      ...
      <nav className="breadcrumb">
        ...
        <span className="breadcrumb-separator">›</span>
        <span className="breadcrumb-item active">Update Nutrition Guide</span>
      </nav>
      ...
      <div className="form-card">
        ...
        <div className="form-header">
          <div className="form-title-section">
            ...
            <div>
              <h1 className="form-title">UPDATE NUTRITION GUIDE</h1>
              <p className="form-subtitle">Edit content, media, and publication info</p>
            </div>
          </div>
        ...
        <form onSubmit={submit} className="registration-form">
          ...
          <div className="form-section">
            <div className="section-header">
              ...
              <p className="section-subtitle">Classification & title</p>
            </div>
          ...
          <div className="form-section">
            <div className="section-header">
              ...
              <p className="section-subtitle">Edit summary & full text</p>
            </div>
          ...
          <div className="form-section">
            <div className="section-header">
              ...
              <p className="section-subtitle">External link & dates</p>
            </div>
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                {errors.published_date && (
                  <span className="error-text">{errors.published_date}</span>
                )}
              </div>
